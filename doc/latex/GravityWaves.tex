...
If we assume that in the preconditioner the buoyancy can be eliminated pointwise, we can save one inversion of $M_b$ and write $(M_u\tilde{\vec{R}}_u)=(M_u\vec{R}_u)+\frac{\Delta t}{2}\Pi_v^T(M_b\vec{R}_b)$ when calculating the RHS for the velocity in the first step. 
\begin{algorithm}[H]
 \caption{Preconditioner for mixed system (\ref{eqn:MixedSystem}); given $\vec{r}_u=M_u\vec{R}_u$, $\vec{r}_p=M_p\vec{R}_p$ and $\vec{r}_b=M_b\vec{R}_b$ calculate $\vec{U}$, $\vec{P}$ and $\vec{B}$.}
 \label{alg:mixed_preconditioner}
 \begin{algorithmic}[1]
   \STATE{Calculate $\tilde{\vec{r}}_u=\vec{r}_u+\frac{\Delta t}{2}QM_b^{-1}\vec{r}_b$} \COMMENT{Modified right hand side for velocity}
   \STATE{Calculate $\tilde{\vec{r}}_p=\vec{r}_p-\frac{\Delta t}{2}c^2D\tilde{M}_u^{-1}\tilde{\vec{r}}_u$} \COMMENT{Modified right hand side for pressure}
  \STATE{Solve $H\vec{P}=\tilde{\vec{r}}_p$ for $\vec{P}$} \COMMENT{Pressure solve}
  \STATE{Calculate $\vec{U}=\tilde{M}_u^{-1}\left(\tilde{\vec{r}}_u+\frac{\Delta t}{2}D^T \vec{P}\right)$} \COMMENT{Velocity backstubstitution}
  \STATE{Calculate $\vec{B}=M_b^{-1}\left(\vec{r}_b-\frac{\Delta t}{2}N^2 Q^T \vec{U}\right)$} \COMMENT{Buoyancy backstubstitution}
 \end{algorithmic}
\end{algorithm}
...
\STATE{$i=k\cdot o_{\text{row}}(I)+m^{(c)}_{\text{row}}(I)$}
       ...
       \FOR{$J=0,\Ndfcol-1$}
         ...
         \STATE{$\overline{A}_{BW^{(A)}i+(j-j_-)}\mapsto\overline{A}_{BW^{(A)}i+(j-j_-)}+\left(L[A]_k\right)_{I,J}$}
       \ENDFOR
     \ENDFOR
   \ENDFOR
 \end{algorithmic}
...
\begin{algorithm}[H]
 \caption{Generalised banded tranpose matrix-matrix multiplication $C=A^TB$}
 \label{alg:GBMMatrixMatrixProductTranspose}
 \begin{algorithmic}[1]
   \STATE{$\overline{C}\mapsto 0$}
   \FOR{$i=0$ \TO $m_A-1$}
     \STATE{$j_- = \lceil (\alpha^{(C)}i-\gamma_+^{(C)})/\beta^{(C)}\rceil$}
     \STATE{$j_+ = \lfloor (\alpha^{(C)}i+\gamma_-^{(C)})/\beta^{(C)}\rfloor$}
     \STATE{$k_- = \lceil (\beta^{(A)}i-\gamma_-^{(A)})/\alpha^{(A)}\rceil$}
     \STATE{$k_+ = \lfloor (\beta^{(A)}i+\gamma_+^{(A)})/\alpha^{(A)}\rfloor$}
     \FOR{$j=\max\{0,j_-\}$ \TO $\min\{m_B-1,j_+\}$}
       \STATE{$s\mapsto 0$}
       \FOR{$k=\max\{0,k_-\}$ \TO $\min\{n_A-1,k_+\}$}
         \IF{$\lceil(\alpha^{(B)}k-\gamma_+^{(B)})/\beta^{(B)}\rceil\le j \le \lfloor(\alpha^{(B)}k+\gamma_-^{(B)})/\beta^{(B)}\rfloor$ \textbf{and} $\lceil(\alpha^{(A)}k-\gamma_+^{(A)})/\beta^{(A)}\rceil\le i \le \lfloor(\alpha^{(A)}k+\gamma_-^{(A)})/\beta^{(A)}\rfloor$}
           \STATE{$j'_- = \lceil (\alpha^{(B)}k-\gamma_+^{(B)})/\beta^{(B)}\rceil$}
           \STATE{$i'_- = \lceil (\alpha^{(A)}k-\gamma_+^{(A)})/\beta^{(A)}\rceil$}
           \STATE{$s = s + \overline{A}_{n_{\text{BW}}^{(A)}k+(i-i'_-)}\overline{B}_{n_{\text{BW}}^{(B)}k+(j-j'_-)}$}
         \ENDIF
       \ENDFOR
       \STATE{$\overline{C}_{n_{\text{BW}}^{(C)}i+(j-j_-)}\mapsto  s$}
     \ENDFOR
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
Here we derive the equations (\ref{eqn:ContinuousEquations}) from first principles. The Navier Stokes equations are
\begin{xalignat}{4}
  \frac{D\vec{u}}{Dt} &= -c_p\theta \nabla \pi - g\hat{\vec{z}}, &
  \frac{D\theta}{Dt} &= 0, &
  \frac{\partial \rho}{\partial t}+\nabla\cdot\left(\rho\vec{u}\right) &= 0, &
  \rho\theta &= \Gamma \pi^\gamma.
\end{xalignat}
Expand around stationary profiles $\pi_0(z)$, $\theta_0(z)$ and $\rho_0(z)$
\begin{xalignat}{4}
  \vec{u}(\vec{x},t) &= \epsilon \vec{u}'(\vec{x},t), &
  \pi(\vec{x},t) &= \pi_0(z) + \epsilon \pi'(\vec{x},t), &
  \theta(\vec{x},t) &= \theta_0(z) + \epsilon \theta'(\vec{x},t), &
  \rho(\vec{x},t) &= \rho_0(z) + \epsilon \rho'(\vec{x},t)
\end{xalignat}
At lowest order in $\epsilon$ the stationary solution satisfies hydrostatic balance 
\begin{equation}
  c_p\theta_0 \partial_z \pi_0 = -g\label{eqn:HydrostaticBalance}
\end{equation}
and the equation of state
\begin{equation}
  \rho_0\theta_0 = \Gamma\pi_0^\gamma.
\end{equation}
The equation of state also implies
\begin{xalignat}{2}
  \frac{\partial_z \rho_0}{\rho_0}
+ \frac{\partial_z \theta_0}{\theta_0}
&= \gamma\frac{\partial_z \pi_0}{\pi_0}, &
  \frac{\rho'}{\rho_0}
+ \frac{\theta'}{\theta_0} 
&= \gamma\frac{\pi'}{\pi_0}.\label{eqn:DifferentialEoS}
\end{xalignat} 
At $\order(\epsilon)$ the momentum equation and the conservation of potential temperature are
\begin{xalignat}{2}
  \frac{\partial\vec{u}'}{\partial t} &= c_p\theta_0 \nabla\pi'
  - c_p\hat{\vec{n}}(\partial_z\pi_0) \theta', &
  \frac{\partial \theta'}{\partial t} + \hat{\vec{n}}\cdot\vec{u}' (\partial_z \theta_0) &= 0.\label{eqn:epsilonMomentumBuoyancy}
\end{xalignat}
If we introduce the buoyancy 
\begin{equation}
  b \equiv g\frac{\theta'}{\theta_0},
\end{equation}
and buoyancy (Brunt-Vais\"{a}l\"{a}) frequency $N$ with
\begin{equation}
