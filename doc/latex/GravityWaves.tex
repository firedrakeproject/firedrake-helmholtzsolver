...
\begin{xalignat}{3}
  \alpha^{(A)} &\mapsto \alpha^{(A)}/\text{gcd}(\alpha^{(A)},\beta^{(A)}), &
  \beta^{(A)} &\mapsto \beta^{(A)}/\text{gcd}(\alpha^{(A)},\beta^{(A)}), &
  \gamma_{\pm}^{(A)} &\mapsto \lfloor\beta^{(A)}/\text{gcd}(\alpha^{(A)},\beta^{(A)})\rfloor
\label{eqn:gcd}
\end{xalignat}
...
\begin{algorithm}
 ...
 \begin{algorithmic}[1]
   ...
   \FORALL{columns}
     \FOR{$L=0$ \TO $n_z-1$}
       \STATE{In each cell, calculate the local stencils of $M_\phi$, $M_{u,z,\text{inv}}$ and $D$ and assemble them into the generalised banded matrices $M_\phi^{(c)}$, $M_{u,z,\text{inv}}^{(c)}$, $D^{(c)}$}
     ...
     \STATE{Calculate the block-banded matrix $H^{(c)}=M_{\phi}^{(c)}+\omega^2\Delta_h^{(c)}+\omega^2 D^{(c)}M_{u,\text{inv}}^{(c)}\left(D^{(c)}\right)^T$}
...
\begin{algorithm}
 ...
 \begin{algorithmic}[1]
   \FORALL{columns}
     \FOR{$L=0$ \TO $n_z-1$}
       \STATE{In each cell, calculate the local stencils of $M_\phi$, $M_u$ and $D$ and assemble them into the generalised banded matrices $M_\phi^{(c)}$, $M_u^{(c)}$, $D^{(c)}$}
     \ENDFOR
     \STATE{Calculate the block-SPAI $M_{u,\text{inv}}^{(c)}$ of $M_{u}^{(c)}$}
     \STATE{Calculate the block-banded matrix $H^{(c)}=M_{\phi}^{(c)}+\omega^2 D^{(c)}M_{u,\text{inv}}^{(c)}\left(D^{(c)}\right)^T$}
     \STATE{Calculate $H^{(c)}_{LU}$, the LU decomposition of $H^{(c)}$}
   \ENDFOR
 \end{algorithmic}
\end{algorithm}
